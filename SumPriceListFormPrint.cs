...
{
    public partial class SumPriceListForm
    {
        // 見出し開始位置(X)
        ...
        // 総額
        private long print_sum_price = 0;

        private bool getAddressFlg = false;

        private AddressInfo adrInfo;

        private List<BillCompanyInfo> companyInfoArray;
...
////////////////////////////////////////////////////////////////////////////////////
        ...
        private void printFunc()
        {
            ...
            gYear = (string)row.Cells[0].Value;
            gMonth = (string)row.Cells[1].Value;
            day = UtilString.getLastDay(gYear, gMonth);
            ...
            try
            {
                companyInfoArray = DbAccessFunc.getCompanyGroupData(gYear, gMonth, ref print_sum_price);
            }
            catch (Exception ee)
            {
            ...
            try
            {
                int num = companyInfoArray.Count / PRINT_LINE;
                if (companyInfoArray.Count % PRINT_LINE > 0)
                {
                    num++;
                }
                ...
                String pdffileName = pdfPath + @"\" + gYear + gMonth + "_表紙.pdf";
                bool pdfFlg = Properties.Settings.Default.Pdf && Directory.Exists(pdfPath);
                if (pdfFlg)
                {
                    printBillCompanyData.PrinterSettings = new PrinterSettings()
                    {
                        // Windows10 から利用可能
                ...
                printBillCompanyData.DefaultPageSettings.Margins = new System.Drawing.Printing.Margins(10, 10, 10, 10);
                printBillCompanyData.Print();
                if (pdfFlg)
                {
        ...
        // 会社単位印刷処理
        /////////////////////////////////////////////////////
        private void printBillCompanyData_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            // 見出しフォント
            ...
            e.Graphics.DrawString(buf.ToString(), f_T, Brushes.Black,
                                  e.MarginBounds.Left + PAGE_NO_X + 55, e.MarginBounds.Top + PAGE_NO_Y,
                                  drawNearFormat);
            ...
            if (print_page == 1)
            {
                ...
                bufSum.Append("税込合計金額    ");
                bufSum.Append("\\");
                bufSum.Append(String.Format("{0:n0}", print_sum_price));
                // 日付
            ...
            // 品名
            e.Graphics.DrawString("会   社   名  (種 類)", f, Brushes.Black,
            ...
            // 摘要
            e.Graphics.DrawString("摘 要", f, Brushes.Black,
            ...
            // データの描画
            for (int i = print_count; i < companyInfoArray.Count; i++)
            {
                BillCompanyInfo info = companyInfoArray[i];
                count++;
                if (count > PRINT_LINE)
                {
                    print_count = i;
                ...
                float hinmeiWidth = e.Graphics.MeasureString(info.name, f).Width;
                if (hinmeiWidth < HEIGHT_LINE_2 + (WIDTH_STAET_DATA_TOP))
                {
                    info.name = Strings.StrConv(info.name, VbStrConv.Wide, 0);
                }

                // 会社名
                e.Graphics.DrawString(Strings.StrConv(info.name, VbStrConv.Wide, 0), f, Brushes.Black,
                                      tmp_height_width_x + WIDTH_STAET_DATA_TOP, tmp_data_y2,
                                      drawNearFormat);
                // 数量(右寄り)
                ...
                e.Graphics.DrawString(String.Format("{0:n0}",info.price), f, Brushes.Black,
                                      tmp_height_width_x - WIDTH_STAET_DATA_TOP, tmp_data_y2,
                                      drawFormat);

                sumPrice = sumPrice + info.price;
